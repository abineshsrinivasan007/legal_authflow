...
# ── Import your existing topic_filter ────────────────────────────────────────
# This is already in your project at core/topic_filter.py
# We only call filter_topics() AFTER extraction — zero changes to extraction logic
try:
    from .topic_filter import filter_topics
    TOPIC_FILTER_AVAILABLE = True
    print("[utils] topic_filter loaded.")
except ImportError:
    TOPIC_FILTER_AVAILABLE = False
    print("[utils] topic_filter not found — skipping filter step.")
...
def extract_topics_and_keywords_with_mistral(html_path: str):
    """
    Main AI extraction path. Returns exactly 6 values always.
    topic_filter is applied AFTER extraction to remove low-value topics.
    The extraction logic itself is completely unchanged.
    """
    _empty = (0, 0, [], [], [], "")
    ...
    try:
        ...
        if len(lean_html.strip()) < 50:
            print("lean_html is empty after filtering — skipping Mistral call")
            return _empty
        ...
        # ── Step 2: filter low-value topics using your topic_filter.py ───────
        # This is the ONLY addition — extraction is completely unchanged above
        if TOPIC_FILTER_AVAILABLE and topics:
            before_count = len(topics)
            topics = filter_topics(topics)
            print(f"[utils] topic_filter: {before_count} → {len(topics)} topics kept")

        print(f"Mistral success: {len(topics)} topics, {len(keywords)} keywords "
...
def extract_topics_and_keywords_from_html(html_path):
    ...
    # Apply filter to fallback topics too
    if TOPIC_FILTER_AVAILABLE and topics:
        topics = filter_topics(topics)
...
def extract_topics_and_keywords_from_json(json_path):
    ...
    # Apply filter to JSON fallback topics too
    if TOPIC_FILTER_AVAILABLE and topics_list:
        topics_list = filter_topics(topics_list)
        ...
        try:
            ...
            if html_path:
                ...
                if getattr(doc, "html_hash", None) == new_hash and doc.topics_count:
                    ...
                    if _is_scanned:
                        print(f"SCANNED PDF: {base_name} — skipping AI pipeline.")
                    ...
                    if FAST_EXTRACT:
                        ...
                        if api_key.strip() and Mistral is not None:
                            extracted_topics, extracted_keywords, topics_list, \
                                keywords_list, mindmap_data, ai_topics_data = \
                                extract_topics_and_keywords_with_mistral(html_path)
